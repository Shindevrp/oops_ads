...
class GradeBook:
    ...
    def what_do_i_need(self, id, grade):
        for student in self.students:
            if student.andrew_id==id:
                res=student.what_do_i_need(grade)
                print(f"Student {id} needs an average of {res:.2f} on the remaining work for a(n) A")
                return
        print(f"Error: No student with id {id} found.")
